...
# from service.customer_rule_service import RuleDataService
# from service.airflow_service import AfService
# from service.ma_trigger_mgr import MaTriggerService
...
def order_filter_to_where(data):
    ...
    if area_store:
        warr.append(f"(store_id IN ( '{area_store}' ) OR store_id IS NULL)")
    if bhv_C:
        warr.append(f"( {bhv_C} )")
    if bhv1:
...
def Exec_RuleResult(uid, owner, rid, sql, cl):
    """
    執行bq並上傳檔案API
    :param rid: rule id
